...
%     disp(['Reading ' fname]);
    load(fname);
    traces_zero=double(traces(first_set_start_index:first_set_end_index,:));
    % remove mean from each trace...
    for row_no = 1:1:size(traces_zero,1)
        traces_zero(row_no, :) = traces_zero(row_no, :) - mean(traces_zero(row_no, :));
    end
    X=X+sum(traces_zero);
    X2=X2+sum(traces_zero.^2);
    n=n+size(traces_zero,1);
...
%     disp(['Reading ' fname]);
    load(fname);
    traces_one=double(traces(second_set_start_index:second_set_end_index,:));
    % remove mean from each trace...
    for row_no = 1:1:size(traces_one,1)
        traces_one(row_no, :) = traces_one(row_no, :) - mean(traces_one(row_no, :));
    end
    X=X+sum(traces_one);
    X2=X2+sum(traces_one.^2);
    n=n+size(traces_one,1);
    clear traces scalars points;
end
...
for i = 1:1:no_attack_files
    ...
    for k = 1:1:no_traces_in_file
        ...
        figure;
        hold on;
        plot(mean_trace_correct(1:50),'b');
        plot(mean_trace_faulty(1:50),'r');
        plot(reduced_trace(1:50),'g');
